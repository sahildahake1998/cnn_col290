...
int main(int argc, char *argv[])
{
	if(argc == 5 or argc == 6 or argc == 7)
	{
		...
		else if((string)argv[1] == "maxPooling_of_matrix" or (string)argv[1] == "avgPooling_of_matrix")
		{
			if(sa % padValue != 0)
			{
				cout << "Such a stride value is not possible because the stride value must devide the input matrix size\n";
				return 0;
			}
			float ansM_Pooling[sa/padValue][sa/padValue];
			float* pointer_ansPooling_matrix = ansM_Pooling[0];
			
			if(isRelu)//  isRelu === maxPooling
			{
				
				maxPooling_of_matrix(a, sa, padValue, pointer_ansPooling_matrix);
				printMatrix(pointer_ansPooling_matrix, sa/padValue, sa/padValue);
			}
			else
			{
				avgPooling_of_matrix(a, sa, padValue, pointer_ansPooling_matrix);
				printMatrix(pointer_ansPooling_matrix, sa/padValue, sa/padValue);
			}
		}
		return 0;
